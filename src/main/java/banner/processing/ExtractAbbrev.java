...
/**
 * This class was adapted from the BioText ExtractAbbrev.java software by Ariel S. Schwartz. See http://biotext.berkeley.edu/software.html.<BR>
 * <BR>
 ...
 * that definition, one per line. A file consisting of short-form/long-form pairs (tab separated) can be specified in tandem with the -testlist option
 * for the purposes of evaluating the algorithm.
 * 
    ...
    private String findBestLongForm(String shortForm, String longForm)
    {
        ...
        for (; sIndex >= 0; sIndex--)
        {
            ...
            if (!Character.isLetterOrDigit(currChar))
                continue;
            while (((lIndex >= 0) && (Character.toLowerCase(longForm.charAt(lIndex)) != currChar)) || ((sIndex == 0) && (lIndex > 0) && (Character
                                                                                                                                                  .isLetterOrDigit(longForm
                                                                                                                                                                           .charAt(lIndex - 1)))))
                lIndex--;
            if (lIndex < 0)
                return null;
